...
const Search: FC = () => {
  ...
  const handleSearch = () => {
    if (inputRef.current) {
      router.push(`/q?term=${inputRef.current.value}`);
    }
  };
...
/>
        ...
        <button
          className="relative z-[2] flex items-center rounded-r bg-primary px-6 py-2.5 text-xs font-medium uppercase
           leading-tight text-while shadow-md transition duration-150 ease-in-out hover:bg-primary-700 hover:shadow-lg focus:bg-primary-700 focus:shadow-lg focus:outline-none 
